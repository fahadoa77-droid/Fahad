...
```
        ...
        for upd in r.get("result", []):
            ...
            # ✅ استخدام chat_id من الرسالة مباشرة
...
if not isinstance(resp, list) or not resp: break
...
sig = ml.ewm(span=9, adjust=False).mean()
hist = ml - sig
if hist.iloc[-1] >= 0 or sig.iloc[-1] < 0 or sig.iloc[-1] < hist.iloc[-1]: return False
c24 = max(int(1440 / entry_min), 1)
peak = ml.iloc[-c24:].max() if len(ml) >= c24 else ml.max()
if peak > 0 and ml.iloc[-1] > peak * 0.20: return False
return True
...
send_telegram(f"🚨 <b>إشارة دخول</b>\n🪙 <b>{symbol}</b>\n🎯 الدخول: <b>{entry_min}m</b>\n💰 السعر: {price:.6g}")
```
...
top = sorted([s for s in resp if s[“symbol”].endswith(“USDT”)], key=lambda x: float(x.get(“quoteVolume”, 0)), reverse=True)[:TOP_SYMBOLS_LIMIT]
